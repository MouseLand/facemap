...
class MainW(QtGui.QMainWindow):
    def __init__(self, images=None):
        ...
        self.colormap = (
            plt.get_cmap("gist_rainbow")(np.linspace(0.0, 1.0, 1000)) * 255
        ).astype(np.uint8)
        self.reset()
            ...
            if (
                ...
                if not self.in_stroke:
                    ...
                    if event.key() == QtCore.Qt.Key_Left:
                        ...
                        self.BrushChoose.setCurrentIndex(
                            max(self.BrushChoose.currentIndex() - 1, 0)
                        )
                        self.brush_choose()
                    elif event.key() == QtCore.Qt.Key_Down:
                        self.BrushChoose.setCurrentIndex(
                            min(self.BrushChoose.currentIndex() + 1, 3)
                        )
                        self.brush_choose()
                self.update_plot()
    ...
    def make_viewbox(self):
        self.p0 = ViewBoxNoRightDrag(
            lockAspect=True, name="plot1", border=[100, 100, 100], invertY=True
        )
        self.brush_size = 3
            ...
            if image.shape[-1] < 3:
                ...
                image = np.concatenate(
                    (
                        image,
                        np.zeros(
                            (shape[0], shape[1], 3 - shape[2]),
                            dtype=type(image[0, 0, 0]),
                        ),
                    ),
                    axis=-1,
                )
    ...
    def compute_saturation(self):
        ...
        self.saturation = [
            np.percentile(self.stack.astype(np.float32), 1),
    ...
    def compute_model(self):
        ...
        for z in range(len(self.stack)):
            ...
            y = y[
                :,
                :,
                pads[-2][0] : y.shape[-2] - pads[-2][-1],
                pads[-1][0] : y.shape[-1] - pads[-1][-1],
            ]
            print(y.shape)
            ...
            self.flows[-1].append(
                np.clip(y[0, -1] * 127 + 127, 0, 255).astype(np.uint8)
            )
                ...
                if mn.sum() > 5:
                    ...
                    if pix is not None:
                        ...
                        median = self.add_mask(
                            points=pix, color=color, mask=mn.nonzero()
                        )
    ...
    def load_images(self, filename=None):
        ...
        if filename is None:
            name = QFileDialog.getOpenFileName(self, "Load image")
            filename = name[0]
        manual_file = os.path.splitext(filename)[0] + "_manual.npy"
        if os.path.isfile(manual_file):
            print(manual_file)
            self.load_manual(
                manual_file, image=io.imread(filename), image_file=filename
            )
            return
        try:
