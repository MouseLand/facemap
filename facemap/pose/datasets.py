...
class OLDMETHOD(torch.utils.data.Dataset):
    def __init__(
        self,
        image_data,
        keypoints_data,
    ...
    def __getitem__(self, item):
        ...
        """
        image, keypoints = self.preprocess_data(
            self.images[item], self.keypoints[item], self.bbox
        )
        ...
        if not isinstance(keypoints, torch.Tensor):
            keypoints = torch.from_numpy(keypoints).to(
    ...
    def preprocess_img(self, image):
        """
        Preprocess images to be used in the model. Preprocessing involves:
            1. Convert to float
            2. Normalize image
        Returns
        -------
        image : ND-arrays of shape ([C], W, H)
            An image.
        """
        ...
        # Normalize
        image = pose_utils.normalize99(image)
        return image
